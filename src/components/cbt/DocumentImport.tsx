...
interface DocumentImportProps {
  onProcessComplete: (files: Record<string, File>) => void
}
...
export default function DocumentImport({ onProcessComplete }: DocumentImportProps) {
  ...
  const processDocuments = async () => {
    ...
    onProcessComplete(uploadedFiles)
    setUploadedFiles({})
    setProcessing(false)
  }
  ...
  return (
    ...
    >
      <h2 className="text-xl font-bold text-slate-900 mb-4">Import Documents</h2>
      <p className="text-slate-600 mb-6 text-sm">Upload your study materials in any format</p>
      
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 mb-6">
        {documentTypes.map((item) => (
          <div key={item.type} className={`p-4 rounded-xl border-2 border-dashed ${item.color} border-opacity-30 relative hover:border-opacity-50 transition-all`}>
            <input
              type="file"
              accept={item.accept}
            ...
            {uploadedFiles[item.type] && (
              <p className="text-sm text-green-600 mt-2 font-medium truncate">✓ {uploadedFiles[item.type].name}</p>
            )}
          </div>
        ))}
      </div>
      ...
      >
        {processing ? (
          ...
          <>
            ...
            Process {Object.keys(uploadedFiles).length} Document{Object.keys(uploadedFiles).length !== 1 ? 's' : ''}
          </>
        )}
      </button>
    </motion.div>
